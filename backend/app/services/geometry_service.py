...
def extract_features_from_mesh(mesh: trimesh.Trimesh) -> Dict[str, Any]:
    """
    Compute all geometric features from an already-loaded Trimesh.
    Use this when the mesh has already been loaded to avoid a redundant disk read.
    """
    # Basic dimensions
...
def convert_to_glb(input_path: str, uuid: str, preloaded_mesh: trimesh.Trimesh | None = None) -> str:
    """
    ...
    mesh = preloaded_mesh if preloaded_mesh is not None else load_mesh(Path(input_path))
